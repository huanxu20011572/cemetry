...
function archivesSelect(){
	...
	action += "&redirect_registry_url="+redirect_registry_url;
	//业务办理类型
  ...
  <tr>
	...
	<form action="cemeteryRegistryController.fj?method=getCemeteryRegistryPageList" method="post">
       ...
       <form action="../archives/archivesCemeteryRegistryController.fj?method=archives" onsubmit="return archives();" method="post">
		  ...
		  <table width="98%" cellpadding="0" cellspacing="1" bgcolor="#a6c6da">
	        ...
	        <tr>
			  ...
			  <a href="cemeteryRegistryController.fj?method=getCemeteryRegistryViewById&id=${en.id}">查看</a>&nbsp;
			  <c:if test="${en.cemeteryLocation.status == 4 }">
				  <a href="cemeteryRegistryController.fj?method=editCemeteryRegistry&cemeteryRegistryType=${en.cemeteryRegistryType}&id=${en.id}">编辑</a>&nbsp;
				  <a href="javascript:deleteCemeteryRegistry('${en.id}','${en.cemeteryRegistryType}');">删除</a>&nbsp;
			  </c:if>
			  <c:if test="${en.cemeteryLocation.status == 3 && en.cemeteryRegistryType == 1}">
				  <a href="cemeteryRegistryController.fj?method=getCemeteryRegistryBack&id=${en.id}">退墓</a></td>
			  </c:if>
			  <c:if test="${en.cemeteryRegistryType == 5}">
				  <a href="cemeteryRegistryController.fj?method=viewBackCemeteryRegistry&id=${en.id}">查看退墓</a></td>
			  </c:if>
	        </tr>
	        </c:forEach>
	      </table>
      ...
      <table width="98%" border="0" cellspacing="0" cellpadding="10" class="word-4">
			<tr align="right" >
				<td >
				<ft:page action="cemeteryRegistryController.fj?method=getCemeteryRegistryPageList" page="${page}">

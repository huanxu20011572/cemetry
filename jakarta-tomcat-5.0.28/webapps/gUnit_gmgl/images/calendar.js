...
{    //parent.document.write("11111")
    ...
    "td#meizzYearHead, td#meizzYearMonth{color: #000080}"+
    ...
    " if ( (window.event.keyCode >= 48 && window.event.keyCode <= 57 )|| (window.event.keyCode == 8)){ window.event.returnValue= true;} "+
    " else{window.event.keyCode = 0; window.event.returnValue= false;}}"+
    ...
    "    <td width=60 id=meizzYearHead  title='点击此处选择年份' onclick='parent.funYearSelect()'"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         strIframe +="<td colspan=2 align=left class=out title='"+ WebCalendar.regInfo +"'><input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' onfocus='this.blur()'"+
         " type=button value='关闭' onclick='parent.hiddenCalendar()' ></td><td class=out title='"+ WebCalendar.regInfo +"'> <input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' type=button value='清' onclick='parent.resetqk()' ></td></tr></table>"+
    ...
    if(WebCalendar.timeShow){
        ...
        strIframe += "<tr><td height=20 width=140 bgcolor='"+ WebCalendar.lightColor +"'>"+
                     "<table border=0 cellpadding=1 cellspacing=0 width=140>"+
                     "    <tr><td>时间： <input name=chour type=text value='" + appendZero(new Date().getHours()) + 
                     "' style='width: 20' onkeyup='parent.changeHour(this,this.value)'>:" +
                     "<input name=cminute type=text value='" + appendZero(new Date().getMinutes()) + 
                     "' style='width: 20' onkeyup='parent.changeMin(this,this.value)'>:" +
                     "<input name=csecond type=text value='" + appendZero(new Date().getSeconds()) + 
                     "' style='width: 20' onkeyup='parent.changeSec(this,this.value)' ></td></tr></table></td></tr>";   
    }
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.time       = new Array(6);             //时间日历定义  luoxinyu 2007-06-11
...
function calendarP(){
    document.all("meizzCalendarLayer").style.height = 218;
    WebCalendar.timeShow = true;
    initParm();
    
    WebCalendar.time[0] = appendZero(new Date().getHours());
    WebCalendar.time[1] = appendZero(new Date().getMinutes()) ;
    WebCalendar.time[2] = appendZero(new Date().getSeconds());
    
    document.all("meizzCalendarLayer").style.height = 218;
}

function calendar(m,e){
	if(m!=null && m > 0){
		document.all("meizzCalendarLayer").style.height = 218;
    WebCalendar.timeShow = true;
    initParm(e);
    ...
    document.all("meizzCalendarLayer").style.height = 193;
...
function initParm(objExport) //主调函数
{
    var e = window.event.srcElement;
    writeIframe();
    var o = WebCalendar.calendar.style; WebCalendar.eventSrc = e;

	if (objExport==null) WebCalendar.objExport = e;
    else WebCalendar.objExport = eval(objExport);
	...
	/** 2007-11-10 ADD BY YQX BEGIN */
	o.top = document.body.scrollTop + event.clientY;
	/** 2007-11-10 ADD BY YQX END */
...
function funMonthSelect() //月份的下拉框
{
    ...
    for (var i=1; i<13; i++){var oOption = WebCalendar.iframe.document.createElement("OPTION");e.options.add(oOption);oOption.innerText=i +"月";oOption.value=i};
...
function funYearSelect() //年份的下拉框
{
    ...
    var max = (y + n <= 9999) ? y + n : 9999;
        min = (max == 9999) ? max-n*2 : min-50;
        max = (min == 1000) ? min+n*2 : max-30;
    for (var i=min; i<=max; i++){var oOption = WebCalendar.iframe.document.createElement("OPTION");e.options.add(oOption);oOption.innerText=i +"年";oOption.value=i;};
...
function changeHour(ObjCon,h){
    ...
    WebCalendar.time[0] = h;
    
    return true;
}

function changeMin(ObjCon,h){
    if(h != null && h.trim().length >0){
        if( h -0 >=0 && h-60>= 0) {
           ObjCon.value ="";
           ObjCon.focus();
    ...
    WebCalendar.time[1] = h;
    
    return true;
}

function changeSec(ObjCon,h){
    if(h != null && h.trim().length >0){
        if( h -0 >=0 && h-60>= 0) {
           ObjCon.value ="";
           ObjCon.focus();
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(WebCalendar.timeShow)
        {
            ...
            var h = WebCalendar.time[0], m = WebCalendar.time[1], s = WebCalendar.time[2];

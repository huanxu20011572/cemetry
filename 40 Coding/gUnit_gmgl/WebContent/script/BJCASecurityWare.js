...
function CAdditionHTML()
{
	...
	this.ItemName[3] = "SecX_ActionSign";
...
document.writeln("<OBJECT classid=\"clsid:21336F5A-8C51-463A-81B9-B5D37CD4D6FB\" height=1 id=BJCASecClt style=\"HEIGHT: 1px; LEFT: 10px; TOP: 28px; WIDTH: 1px\" width=1 VIEWASTEXT>");
...
function gEncrypt(sInData)
{
	...
	{
		//新版版证书应用加密
		return envelopedData(sInData, ENVELOP_ENC, g_objSI.SvrCert);
	}
	else if(globalAppType==0)
	{
		//旧版版证书应用加密
...
function gSign(sInData)
{
	...
	{
		//新版版证书应用签名
		return signedData(sInData, g_objSI.Container);
	}
	else if(globalAppType==0)
	{
		//旧版版证书应用签名
...
function gFuncSubmit(strFormName)
{
	...
	document.all("SecX_Form").SecX_ActionType.value = "Both";//"",Sign,Encrypt,Both
	document.all("SecX_Form").SecX_SignData.value = gSign(strToPost);
	document.all("SecX_Form").SecX_UserCert.value = gGetCert();
	document.all("SecX_Form").SecX_EncData.value = gEncrypt(strToPost,g_objSI.SvrCert);
...
function doSecurityXSubmit(form, uniqueID) {
	if(form == null) {
		alert("表单对象不能创建成功!");
		return false;
	}
	if(uniqueID == null || uniqueID == "") {
		...
		alert(form);
    		...
    		if (!objForm.clientsigndata)
    		{
            		objForm.insertAdjacentHTML("BeforeEnd",g_objASecH.HTMLItemString)
    		}
    		else
    		{
            		objForm.secClientSignData.value = "";
            		objForm.secOrginData.value = "";
            		objForm.secClientSignCert.value = "";
            		objForm.secActionSign.value = "";
    		}
    		...
    		for (i = 0;i < objForm.length ; i++ )
    		{
            		for (j = 0; j < 10; j++)
            		{
                    		if (objForm.item(i).type == g_objF.ReIType[j] && (objForm.item(i).name.indexOf("sec") < 0))
                    		{
                            		strToSign += objForm.item(i).name + "="+objForm.item(i).value+",";
                    		}
            		}
    		}

    		objForm.secOrginData.value=strToSign;
    		objForm.secClientSignData.value = gFuncSign(strToSign);
    		objForm.secClientSignCert.value = gFuncGetDetailNoInit(uniqueID, 0);
		objForm.secActionSign.value = "Sign";
		alert("原始数据：" + objForm.secOrginData.value);
    		alert("签名数据：" + objForm.secClientSignData.value);
    		alert("用户证书：" + objForm.secClientSignCert.value);
		objForm.submit();
	}
	return true;
}
...
function Login(strFormName,strContainerName,strPin) {
	...
	if (strDeviceType == "BJSOFT") {
		...
		var strClientSignedData = signedData(g_objSI.SvrOriData, strContainerName);
	}
	else if (strDeviceType == "BJCSP0001"){
		//软CSP
		var strDevType = getUserInfoByContainer(strContainerName, CERT_XML_DEVICETYPE);
	...
	objForm.SecX_UserCert.value = exportCert(CERT_DST_BASE64);
...
/*
**************************************************
Title:			BJCA SecX from SecFuncLib.js
Version:		2.1
Author:			HeMeng, hemeng@bjca.org.cn
	                sunhongbin doSecurityXSubmit
Date:			2003/12/08
...
function gFuncLogin(strFormName, strUniqueID, strPassword)
{
	...
	switch(ret)
	{
		...
		case 0:
			...
			objForm.SecX_SignData.value = gFuncSign(strServerRan);

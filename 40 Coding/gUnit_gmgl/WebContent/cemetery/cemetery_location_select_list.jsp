	...
	<table width="98%" > 
		<tr> <td>
			<c:forEach  var="en" items="${list}" varStatus="status">
				 ...
				 " 
				 onclick="selectCemeteryLocation('<c:out value='${en.cemeteryType.id}'/>','<c:out value='${en.id}'/>','<c:out value='${en.cemeteryZone.parent.cemeteryZoneName}'/><c:out value='${en.cemeteryZone.cemeteryZoneName}'/><c:out value='${en.cemeteryLocationCode}'/>号');"
	          	>
...
<!-- 	图例   -->
	...
	<c:if test="${param.pid > 0}" >
		<table width="98%" border="0" cellspacing="0" cellpadding="10" class="word-4">
			<tr align="right" >
				<td >
				<ft:page action="cemeteryLocationController.fj?method=getCemeteryLocationSelectListByPID" page="${page}">

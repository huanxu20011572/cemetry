...
function CheckDate(oIn, clean){
    ...
    if (v == null || v == ""){
        ...
        alert("请输入日期 2005-05-16 (年/月/日)");
        oIn.focus();
        return;
    }
    if (!isDate(oIn.value)){
        alert("请输入有效的日期格式 2005-05-16 (年/月/日)");
        oIn.focus();
        oIn.select();
    }
}

function InputCalendar(name, refDate, starYear, endYear, width, height, border, defaultDay, clean, dayOffset){
	this.refDate = refDate;
	this.starYear = starYear;
	this.endYear = endYear;
	this.width = width;

  ...
  <tr> 
    <td valign="top"> 
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table7">
	    ...
	    <toolbar>
  			<item title="查询日志" icon="../../../images/config/zoom_small.gif"          action="location.href='log_search.jsp'"/>	        
	    	<item title="批量删除"   icon="../../../images/config/delete.gif"      action="myform.actionType.value='deleteAll'; onSubmit();"/>   	
	    	<break/> 
	...
	<span id="os_toolbar_2727376791_result" style="display: inline-block;"></span> 
      ...
      <form action="delete.do?action=delete" name="myform" method="post">  
	...
	<c:if test="${outterstatus.count % 2 == 1}">
        ...
        <c:if test="${outterstatus.count % 2 != 1}">
	      ...
	      <c:forEach var="cell" items="${row}" varStatus="innerstatus">
	        <c:if test="${innerstatus.count == 1}">
	         ...
	         <td width="5%" height="21">
	      	...
	      	<TD>
	      	<c:if test="${innerstatus.count==4}">
	      		<a href="logSelect.do?action=detail&id=<c:out value="${identity}"/>">
        	</c:if>
        		<c:out value="${cell}"/>
	      	<c:if test="${innerstatus.count==4}">
	      	</a>
          ...
          <td width="12%">   
            <a href=# onclick="myform.actionType.value='delete'; myform.deleteId.value='<c:out value="${identity}"/>'; myform.submit();">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table7">	      
        <tr> 
           <form name="checkall">
            ...
            href="javascript:CheckAll();">选中所有的日志</A> 
             
        </td>
		 </form>         
          <td width="72%" valign="bottom"> 
            <script src="../../../script/function.js"></script>
  <gOS:page name="test" action="logSelect.do?action=searchResult" pageName="queryPage" pageSizeKey="query">
     <c:forEach var="item" items="${itemList}" >
        <input name="<c:out value='${item.name}'/>" value="<c:out value='${item.value}'/>" type="hidden">
     </c:forEach> 
  </gOS:page> </td>

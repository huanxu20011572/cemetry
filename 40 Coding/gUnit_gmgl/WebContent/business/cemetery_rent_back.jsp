...
function doCheck(){
	...
	if(is_oneself == 0){
	    ...
	    if(agentTel!=""&&!patrn_num.exec(agentTel)){
	    	alert("代理人电话为数字,请重新输入");
	    	return false;
	    }
	   var agentProve=$.trim($("#agentProve").val());
		if(agentProve==""){
		    alert("代理凭证不能为空");
	    	return false;
		}	
	}
	var backPrice=$.trim($("#backPrice").val());
	if(backPrice!=""&&!patrn.exec(backPrice)){
		alert("退墓金额只能为货币!");
		return false;
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
  <tr>
    ...
    <td align="center" valign="top" >
	  ...
	  <form id='saveFileForm' onsubmit="return doCheck();"action="cemeteryRegistryController.fj?method=backCemeteryRegistry" method='post'>
	  <input name="id" type="hidden" id="id" value="${registry.id}"/>
        ...
        <tr>
          ...
          <td  colspan='3'width="35%" align="left" bgcolor="#FFFFFF">
            <input name="isOneself" type="radio" id="isOneself"  checked="checked" onClick="javascirpt:oneself()"  value="1" />是
            <input name="isOneself" type="radio" id="isOneself"  onClick="javascirpt:oneself()"  value="0" / />否
         </td>
        </tr>
        <tr id="no_oneself">
			...
			<tr>
	          ...
	          <td height="35" colspan="3" align="left" bgcolor="#FFFFFF"><textarea name="backReason" style='height: 30px; width: 90%'  ></textarea></td>
            </tr>
	        <tr>

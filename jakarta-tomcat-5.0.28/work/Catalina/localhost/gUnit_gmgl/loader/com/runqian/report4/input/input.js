	...
	switch( editStyle ) {
		...
		case "3":
			...
			if( cell.editConfig != null ) {
				...
				if( editor.configs != configs ) {
					...
					editor.clearOptions();
					if( !editor.isMulti ) editor.addOption( "", "" );
		...
		default:
			try {
				...
				if( pos > 0 ) mainName = mainName.substring( 0, pos );
				var div_input = document.getElementById( mainName + "_reportDiv" );
				div_input.insertAdjacentHTML( "beforeEnd", "<input type=text id=\"" + table.id + "_editBox\" style=\"position:absolute;" + border + ";display:none;zIndex:100\" >" );
				editor = eval( table.id + "_editBox" );
				editor.onkeyup = _editorKeyPress;
			}
...
function _uploadFile( url, table, ds, tblName, keyCols, keyCells, origins, fields, valueCells, currCell, ext, rights, fileNameCol ) {
	...
	url += "&ds=" + _urlEncode( ds ) + "&data=" + _urlEncode( data ) + "&originData=" + _urlEncode( origins ) + "&ext=" + _urlEncode( ext )
			...
			+ "&cells=" + _urlEncode( valueCells ) + "&currCell=" + currCell + "&keyCells=" + keyCells + "&right=" + rights + "&fileNameCol=" + fileNameCol;
	window.open( url, "_blank", "left=200, top=250, height=180, width=400, status=no, resizable=yes" );
}
...
function _uploadPrompt( display, event ) {
	...
	try {
		...
		document.body.insertAdjacentHTML( "AfterBegin", "<table id=___promptBox style=\"display:none; BACKGROUND-COLOR: mistyrose; BORDER: blue 1px solid; FONT-FAMILY: 宋体; FONT-SIZE: 13px; POSITION: absolute;\">" +

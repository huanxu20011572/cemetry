...
//	sb.append("comQuery.savePath='" + this.getSavePath() + "';");
	...
	addRow : function() {
		...
		tr.insertCell(-1).innerHTML = '<select id="field_' + comQuery.maxId + '" style="width:100%" onchange="comQuery.changeField(' + comQuery.maxId + ');">' + comQuery.getFieldOptions() + '</select>';
		tr.insertCell(-1).innerHTML = '<select id="operator_' + comQuery.maxId + '" style="width:100%" onchange="comQuery.changeOperator(' + comQuery.maxId + ');">' + comQuery.getOperators(comQuery.fields[0]) + '</select>';
		tr.insertCell(-1).innerHTML = comQuery.getInputTexts(comQuery.maxId);
...
*/
	...
	switchMode : function(askFlag) {
		...
		if ($("switchBut").innerHTML == comQuery.resource.res2) {
			$("switchBut").innerHTML = comQuery.resource.res1;
			$("conditionTable").width = 580 + comQuery.fieldColWidth + "px";
			...
			$("switchBut").innerHTML = comQuery.resource.res2;
			$("conditionTable").width = 410 + comQuery.fieldColWidth + "px";

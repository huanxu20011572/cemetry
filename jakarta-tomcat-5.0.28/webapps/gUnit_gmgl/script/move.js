...
+------------------------------------------------------------------------
*/
function mainDiv_onmousemove(){	
	event.cancelBubble = true;
	listTable.childNodes(0).childNodes(0).style.cursor="default"
    var curObj=listTable.document.elementFromPoint(event.x,event.y)
	if (blnAdjust==true){
		line.style.left =event.x+listTable.document.body.scrollLeft
	}else{
	   if(curObj==null)return
	   ...
	   if(blnTmp1){
        listTable.rows(curRow).style.cursor="default";
		var cellLength = listTable.rows(curRow).cells.length
	    for (var i=0;i<cellLength-1;i++){
		  if(listTable.childNodes(0).childNodes(i).style.display!="none"){
		    if (event.x+listTable.document.body.scrollLeft>=parseInt(tableContainer.offsetLeft)+parseInt(listTable.rows(curRow).cells(i).offsetLeft)+parseInt(listTable.rows(curRow).cells(i).offsetWidth-4)
				&&event.x+listTable.document.body.scrollLeft<=parseInt(tableContainer.offsetLeft)+parseInt(listTable.rows(curRow).cells(i).offsetLeft)+parseInt(listTable.rows(curRow).cells(i).offsetWidth+4)){  
			      if(listTable.border=="0"){
				    var iTmpLeft=parseInt(listTable.rows(curRow).cells(i).offsetLeft)+1
				  }else{
					var iTmpLeft=parseInt(listTable.rows(curRow).cells(i).offsetLeft)
				  }
				  var offsetCol=LeftToCol(iTmpLeft,listTable)
				  if(event.x+listTable.document.body.scrollLeft>=parseInt(tableContainer.offsetLeft)+iTmpLeft+parseInt(listTable.rows(curRow).cells(i).offsetWidth)) {
					var bFind=false
					for (var j=offsetCol+1;j<cellLength;j++){
					  if(listTable.childNodes(0).childNodes(j).style.pixelWidth>1){
						break
					  }
					  bFind=true
					}
				  ...
				  listTable.rows(curRow).style.cursor ="move"
				  break
				}
			}
		}
